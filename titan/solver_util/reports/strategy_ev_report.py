...
class _StrategyEvTableEntry:

    __slots__ = ('_hand', '_frequencies', '_evs')

    def __init__(self, hand: str, frequencies: typing.Tuple[float, ...], evs: typing.Tuple[float, ...]):
        self._hand = hand
    ...
    def evs(self) -> typing.Tuple[float, ...]:
        return self._evs

    def __str__(self):
        ev_columns = '\t\t'.join((f"{freq * 100:>5.1f}% EV={ev:<7}".ljust(20) for freq, ev in zip(self.frequencies(), self.evs())))
        return f"{self.hand().ljust(4)}\t\t{ev_columns}"


class _StrategyEvTable:
    
    __slots__ = ('_strategy_options', '_entries')

    MAX_FREQUENCY = 10000

    def __init__(self, strategy_options: typing.Tuple[StrategyOption, ...], entries: typing.Tuple[_StrategyEvTableEntry, ...]):
    ...
    def entries(self) -> typing.Tuple[_StrategyEvTableEntry, ...]:
    ...
    def __str__(self):
        if self.is_leaf():
            return f"(No table available)"
        header_items = ('HAND', ) + tuple(f"{str(opt).ljust(20)}" for opt in self.strategy_options())
        spacer_row = "-" * 30 * len(header_items)     
    ...
    def create_from_solution_tree_node(cls, solution_tree_node: SolutionTreeNode):
        ...
        ev_matrix = solution_tree_node.solved_spot().ev_matrix()
        ...
        if options == ():
            ...
            entries = ( _StrategyEvTableEntry(  hand=PreflopRangeMap.hand_for_index(i),
                                                frequencies=tuple(strategy_matrix.values()[i] / cls.MAX_FREQUENCY),
                                                evs=ev_matrix.values()[i]  )
            ...
            entries = ( _StrategyEvTableEntry(  hand=PostflopRangeMap.hand_for_index(i),
                                                frequencies=tuple(strategy_matrix.values()[i] / cls.MAX_FREQUENCY),
                                                evs=ev_matrix.values()[i]  )
...
class _ReportNode:
    ...
    def __init__(self, strategy_table: _StrategyEvTable, children: typing.Dict[str, _ReportNode]):
        self._strategy_table = strategy_table
        self._children = dict(children)

    def strategy_table(self) -> _StrategyEvTable:
    ...
    def save_to_filesystem(self, path: str):
        ...
        # save strategy table
        with open(cur_path / 'strategy-and-evs.txt', 'w') as f:
    ...
    def create_from_solution_tree_node(cls, solution_tree_node: SolutionTreeNode, children: typing.Dict[str, _ReportNode]):
        strategy_table = _StrategyEvTable.create_from_solution_tree_node(solution_tree_node)
        return cls(strategy_table, [])


class StrategyEvStreetReport:
    ...
    @classmethod
    def create(cls, solution_tree: SolutionTree) -> StrategyEvStreetReport:
...
class StrategyEvReport:

    __slots__ = ('_strategy_ev_street_reports',)

    def __init__(self, strategy_ev_street_reports: typing.Tuple[StrategyEvStreetReport, ...]):
        self._strategy_ev_street_reports = strategy_ev_street_reports

    def save_to_filesystem(self, path: str):        
        sub_report_names = ('preflop', 'flop', 'turn', 'river')
        cur_path = pathlib.Path(path)
        for sub_report_name, report in zip(sub_report_names, self._strategy_ev_street_reports):
    ...
    def create(cls, hand_history: HandHistory, solution_trees: typing.Tuple[SolutionTree, ...]) -> StrategyReport:
        return cls(strategy_ev_street_reports=tuple(StrategyEvStreetReport.create(solution_tree)
