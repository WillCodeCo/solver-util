...
class PreflopSolverBenchmarkEntry:

    __slots__ = (   '_solver_config',
                    '_action_sequences',
                    '_solved_spot_extracts' )

    def __init__(self, solver_config: PreflopSolverConfig, action_sequences: typing.Tuple[ActionSequence, ...], solved_spot_extracts: typing.Tuple[SolvedSpotExtract, ...]):
        self._solver_config = solver_config
        self._action_sequences = action_sequences
        self._solved_spot_extracts = solved_spot_extracts

    def solver_config(self) -> PreflopSolverConfig:
    ...
    @classmethod
    def create_from_dict(cls, some_dict: dict) -> PreflopSolverBenchmarkEntry:
        try:
            return cls( solver_config=PreflopSolverConfig.create_from_dict(some_dict['solver_config']),
...
class PreflopSolverBenchmark:

    __slots__ = (   '_entries' )

    def __init__(self, entries: typing.Tuple[PreflopSolverBenchmarkEntry, ...]):
        self._entries = entries

    def entries(self) -> typing.Tuple[PreflopSolverBenchmarkEntry, ...]:
    ...
    @classmethod
    def create_from_dict(cls, some_dict: dict) -> PreflopSolverBenchmark:
        try:
            return cls(entries=tuple(PreflopSolverBenchmarkEntry.create_from_dict(entry_dict) for entry_dict in some_dict['entries']))
        except KeyError as e:
            raise ValueError(f"{cls.__name__}.create_from_dict() failed:  Missing key `{e}`")
...
class _SpotModelHelper:
    ...
    def create_new_street_spot(cls, seat_stacks: typing.Tuple[SeatStack],
        ...
        return PreflopBettingRound.create_initial_spot( seat_stacks=seat_stacks,
                                                        button_assignment=button_assignment,
                                                        blind_bet_sequence=blind_bet_sequence )
    @classmethod
    def create_seat_stacks(cls, solver_config: PreflopSolverConfig) -> typing.Tuple[SeatStack]:
        return tuple((  SeatStack(  seat=seat,
                                    stack_size=stack_size  )
                            for seat, stack_size in enumerate(solver_config.deal_order_stack_sizes())  ))

    @classmethod
    def create_button_assignment(cls, solver_config: PreflopSolverConfig) -> ButtonAssignment:
    ...
    @classmethod
    def create_initial_spot(cls, solver_config: PreflopSolverConfig):
...
class PreflopSolverBenchmarkFactory:
    ...
    def create_solved_spot_extract(cls, spot: Spot, solved_spot: SolvedSpot, deal_order_hole_cards: typing.Tuple[str, ...]):
        ...
        preflop_hand = HandComboMap.preflop_hand_for_combo(deal_order_hole_cards[acting_seat])
        return SolvedSpotExtract.create_from_solved_spot(   solved_spot=solved_spot,
                                                            combo_indexes=(PreflopRangeMap.index_for_hand(preflop_hand),)  )

    @classmethod
    def create_entry_from_solution_tree(cls, solver_config: PreflopSolverConfig, solution_tree: SolutionTree, deal_order_hole_cards: typing.Tuple[str, ...]) -> PreflopSolverBenchmarkEntry:
        ...
        return PreflopSolverBenchmarkEntry(solver_config=solver_config,
    ...
    def add_benchmark_entry(self, solver_config: PreflopSolverConfig, solution_tree: SolutionTree, deal_order_hole_cards: typing.Tuple[str, ...]):
        self._benchmark_entries.append(self.create_entry_from_solution_tree(solver_config, solution_tree, deal_order_hole_cards))

    def preflop_solver_benchmark(self):
        return PreflopSolverBenchmark(tuple(self._benchmark_entries))
